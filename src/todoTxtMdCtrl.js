...
function plugin(CodeMirror) {
	const mode = {
		start: [
			...
			{ regex: /(\@[^@\s]\S*)/, sol: true, token: "todo-context" },
			{ regex: /(\s+)(\@[^@\s]\S*)/, token: [null,"todo-context"] },
			{ regex: /(\+[^\+\s]\S*)/, sol: true, token: "todo-project" },
			{ regex: /(\s+)(\+[^\+\s]\S*)/, token: [null,"todo-project"] },
			{ regex: /([A-Za-z][A-Za-z0-9]*:\S+)/, sol: true, token: ["todo-meta"] },
			{ regex: /(\s+)([A-Za-z][A-Za-z0-9]*:\S+)/, token: [null, "todo-meta"] },
			{ regex: /([1-2]\d{3}[-/\\.](0?[1-9]|1[012])[-/\\.](0[1-9]|[12]\d|3[01]))\b/, token: "todo-date" },
